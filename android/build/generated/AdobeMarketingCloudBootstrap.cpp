...
#define TAG "co.uk.devpulse.adobemarketingcloud"

using namespace v8;

static Persistent<Object> bindingCache;

static void AdobeMarketingCloud_getBinding(const FunctionCallbackInfo<Value>& args)
{
	Isolate* isolate = args.GetIsolate();
	EscapableHandleScope scope(isolate);

	if (args.Length() == 0) {
		titanium::JSException::Error(isolate, "AdobeMarketingCloud.getBinding requires 1 argument: binding");
	...
	titanium::bindings::BindEntry *extBinding = ::AdobeMarketingCloudBindings::lookupGeneratedInit(
...
static void AdobeMarketingCloud_init(Local<Object> exports, Local<Context> context)
{
	...
	Local<FunctionTemplate> constructor = FunctionTemplate::New(isolate, AdobeMarketingCloud_getBinding);
	exports->Set(String::NewFromUtf8(isolate, "getBinding"), constructor->GetFunction(context).ToLocalChecked());
}

static void AdobeMarketingCloud_dispose(Isolate* isolate)
{
	HandleScope scope(isolate);
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			::AdobeMarketingCloudBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry AdobeMarketingCloudBinding = {
	"co.uk.devpulse.adobemarketingcloud",
	AdobeMarketingCloud_init,
	AdobeMarketingCloud_dispose
};

// Main module entry point
extern "C" JNIEXPORT void JNICALL
Java_co_uk_devpulse_adobemarketingcloud_AdobeMarketingCloudBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("co.uk.devpulse.adobemarketingcloud", &AdobeMarketingCloudBinding);
	titanium::KrollBindings::addExternalLookup(&(::AdobeMarketingCloudBindings::lookupGeneratedInit));
}
